...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    // schema may be defined only in the top schema, but not be accessible via
    // topSchema[name]. note that this could probably also be fixed in topSchema setup. 
    var childSchemaObject = topSchema && name ? self.findChildSchemaObject(topSchema, name) : null;

    if (topSchema && topSchema[name + '[]'])
      name = name + '[]';
    
    stack.push({name: originalName, object: obj, schema: (xsiTypeSchema || (topSchema && topSchema[name]) || childSchemaObject), id: attrs.id, nil: hasNilAttribute});
  };
  ...
  p.onclosetag = function(nsName) {
    ...
    if (topSchema && topSchema[name + '[]'] || (cur.schema && cur.schema.$maxOccurs)) {
      if (!topObject[name]) {
        topObject[name] = [];
      }
  ...
  p.ontext = function(text) {
    ...
    // if type is given via the schema...
    var $type = top.schema && top.schema.$type ? top.schema.$type.replace("xs:","") : null;

    if (name === 'int' || name === 'integer' || $type == 'int' || $type == 'integer') {
      value = parseInt(text, 10);
    } else if (name === 'bool' || name === 'boolean' || $type == 'bool' || $type == 'boolean') {
      value = text.toLowerCase() === 'true' || text === '1';
    } else if (name === 'dateTime' || name === 'date' || $type == 'date' || $type == 'dateTime') {
      value = new Date(text);
    } else {
...
WSDL.prototype.findChildSchemaObject = function(parameterTypeObj, childName, visitedBases) {
  if (!parameterTypeObj || !childName) {
    return null;
  }

  /* prevent infinite recursions when $base references create cycles. */
  visitedBases = visitedBases || [];
  if (visitedBases.indexOf(parameterTypeObj)>-1) {
    return null;
  }
  visitedBases.push(parameterTypeObj);
  ...
  if (object.$type) {
    ...
    if (typeDef) {
      return this.findChildSchemaObject(typeDef, childName, visitedBases);
    }
  }

  if (object.children) {
    for (i = 0, child; child = object.children[i]; i++) {
      found = this.findChildSchemaObject(child, childName, visitedBases);
      if (found) {
        break;
      }
      ...
      if (child.$base) {
        ...
        if (foundBase) {
          found = this.findChildSchemaObject(foundBase, childName, visitedBases);
